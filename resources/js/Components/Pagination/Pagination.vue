    ...
    <div class="myPrimarySection" v-if="links && links.length > 3">
        ...
        >
            <template v-for="(link, k) in links" :key="k">
                <div
                    v-if="link.url === null"
                    class="relative inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20 border-none bg-gray-200 text-myPrimaryDarkGrayColor rounded-full"
                    v-html="link.label"
                />
                <Link
                    v-else
