...
class PageBuilder {
  constructor(store) {
    ...
    this.getLocalStorageItemNameUpdate = computed(
      () => this.store.getters['pageBuilderState/getLocalStorageItemNameUpdate']
    );
  ...
  setEventListenersForElements = () => {
    ...
    pagebuilder.querySelectorAll('section *').forEach(async (element) => {
      ...
      if (
        !['P', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(element.tagName)
      ) {
        if (
  ...
  handleRemoveClasses(userSelectedClass) {
    ...
    if (this.getElement.value.classList.contains(userSelectedClass)) {
      ...
      this.store.commit('pageBuilderState/setElement', this.getElement.value);
      this.store.commit('pageBuilderState/removeClass', userSelectedClass);
    }
  }
  ...
  handleDeleteElement() {
    ...
    this.store.commit('pageBuilderState/setParentElement', element.parentNode);
    // Store the outerHTML of the deleted element
    this.store.commit('pageBuilderState/setRestoredElement', element.outerHTML);
    // Store the next sibling of the deleted element
    this.store.commit('pageBuilderState/setNextSibling', element.nextSibling);
  ...
  handleFontSize(userSelectedFontSize) {
    ...
    if (userSelectedFontSize !== undefined) {
      if (
        ...
        this.store.commit('pageBuilderState/setFontBase', userSelectedFontSize);
      }
  ...
  handleCustomBackgroundColor(userSelectedColor, enabledCustomColor) {
    ...
    if (userSelectedColor === undefined && enabledCustomColor === undefined) {
      ...
      if (typeof bgColor === 'string' && bgColor.length !== 0) {
        ...
        this.store.commit('pageBuilderState/setBackgroundColorCustom', bgColor);
      }
  ...
  handleCustomTextColor(userSelectedColor, enabledCustomColor) {
    ...
    if (userSelectedColor === undefined && enabledCustomColor === undefined) {
      ...
      if (typeof textColor === 'string' && textColor.length !== 0) {
        this.store.commit('pageBuilderState/setEnabledCustomColorText', true);
        this.store.commit('pageBuilderState/setTextColorCustom', textColor);
      }

      // Check for inline background color
      if (typeof textColor === 'string' && textColor.length === 0) {
        this.store.commit('pageBuilderState/setEnabledCustomColorText', false);
  ...
  removeCustomColorBackground() {
    ...
    this.store.commit('pageBuilderState/setEnabledCustomColorBackground', null);
    this.store.commit('pageBuilderState/setBackgroundColorCustom', null);
    this.store.commit('pageBuilderState/setElement', this.getElement.value);
  }
  ...
  #addHyperlinkToElement(hyperlinkEnable, urlInput, openHyperlinkInNewTab) {
    ...
    if (hyperlinkEnable === true && typeof urlInput === 'string') {
      ...
      if (hyperlink !== null && urlInput.length !== 0) {
        ...
        this.store.commit('pageBuilderState/setElementContainsHyperlink', true);
    ...
    if (hyperlinkEnable === false && urlInput === 'removeHyperlink') {
      ...
      this.store.commit('pageBuilderState/setElementContainsHyperlink', false);
    }
  }
  ...
  #checkForHyperlink(hyperlinkEnable, urlInput, openHyperlinkInNewTab) {
    ...
    if (hyperlink !== null) {
      ...
      this.store.commit('pageBuilderState/setElementContainsHyperlink', true);
      this.store.commit('pageBuilderState/setHyperlinkInput', hyperlink.href);
      this.store.commit('pageBuilderState/setHyperlinkMessage', null);
      this.store.commit('pageBuilderState/setHyperlinkError', null);

      if (hyperlink.target === '_blank') {
        this.store.commit('pageBuilderState/setOpenHyperlinkInNewTab', true);
      }
      if (hyperlink.target !== '_blank') {
        this.store.commit('pageBuilderState/setOpenHyperlinkInNewTab', false);
      }
